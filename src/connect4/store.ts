...
import { Connect4Battle } from './types'
import { Run } from '../common/types'

const makeRedisBattleId = (battleId: string) => {
  return `arena:connect4:battle:${battleId}`
}

const makeRedisRunId = (runId: string) => {
  return `arena:connect4:run:${runId}`
}

const makeChannel = (channelName: string, battleId: string) => {
  return `areana:connect4:channel:${channelName}:${battleId}`
}

const makeRedisTimerKey = (battleId: string) => {
  return `arena:connect4:timer:${battleId}`
}

const makeRedisLockId = (battleId: string) => {
  return `arena:connect4:lock:${battleId}`
}

export const getBattle = async (
  redis: Redis, battleId: string
): Promise<Connect4Battle | null> => {
  const text = await redis.get(makeRedisBattleId(battleId))
  if (text === null) {
    return null
  }
  return JSON.parse(text) as Connect4Battle
}

export const setBattle = async (redis: Redis, battle: Connect4Battle): Promise<void> => {
  await redis.set(
